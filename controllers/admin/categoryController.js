...
export const editCategory = async (req, res, next) => {
    ...
    try {                
        ...
        } else if (error.message.includes("is shorter than the minimum allowed length (3)")) {
            hasError = true;
            message = "Name should be between 3 to 20 characters.";
...
export const desableCategory = async (req, res, next) => {
    try {
        ...
        await Category.findByIdAndUpdate(req.params.id, { $set: { disabled: state } });
        res.redirect('/admin/categories/1');
    } catch (error) {
        next(error);
    }
...
export const offerAction = async (req, res, next) => {
    try {
        ...
        if (!currentCategory.offerValidUpto || currentDate > currentCategory.offerValidUpto) {
            req.session.message = {
                type: 'danger',
        ...
        await Category.findByIdAndUpdate(req.params.id, { $set: { isOfferActive: state } });
        res.redirect('/admin/categories/1');
