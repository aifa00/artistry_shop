...
export const getDashboard = async (req, res, next) => {
  try {
    ...
    const currentMonthAnalysis = await Order.aggregate([
      ...
      },
      {
        $facet: {
          ...
          totalOrdersThisMonth: [{ $count: "count" }],
    ...
    const todaysOrders = currentMonthAnalysis[0].todaysOrders[0]
      ? currentMonthAnalysis[0].todaysOrders[0].count
      : 0;

    const totalOrdersThisMonth = currentMonthAnalysis[0].totalOrdersThisMonth[0]
      ? currentMonthAnalysis[0].totalOrdersThisMonth[0].count
      : 0;

    const totalCustomersThisMonth = currentMonthAnalysis[0]
      .totalCustomersThisMonth[0]
      ? currentMonthAnalysis[0].totalCustomersThisMonth[0].count
      : 0;

    const totalRevenueThisMonth = currentMonthAnalysis[0]
      .totalRevenueThisMonth[0]
      ? currentMonthAnalysis[0].totalRevenueThisMonth[0].total
      : 0;
...
export const getSingleOrder = async (req, res, next) => {
  try {
    ...
    const currentOrder = await Order.findById(req.params.orderId).populate(
      "products.product"
    );
    const otherProducts = currentOrder.products.filter(
...
export const getSalesReport = async (req, res, next) => {
  try {
    ...
    const totalOrders = salesReportAnalysis[0].totalOrders[0]
      ? salesReportAnalysis[0].totalOrders[0].total
      : 0;
    let salesReport = salesReportAnalysis[0].salesReport;
    const totalRevenue = salesReportAnalysis[0].totalRevenue[0]
      ? salesReportAnalysis[0].totalRevenue[0].total
      : 0;
    const totalProductSold = salesReportAnalysis[0].totalProductSold[0]
      ? salesReportAnalysis[0].totalProductSold[0].total
      : 0;
...
export const downloadSalesReport = async (req, res, next) => {
  try {
    ...
    if (req.query.format === "pdf") {
      ...
      doc
        .fontSize(10)
        .text(`Date: ${new Date(Date.now()).toLocaleDateString()}`, {
          align: "right",
        });
