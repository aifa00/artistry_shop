...
export const getProducts = async (req, res, next ) => {
    ...
    let foundProducts = await Product.find({}).populate('category').skip(skip).limit(pageSize);
...
export const addNewProduct = async (req, res, next) => {
    const { title, category, description, price, stock, artist, color, images, offerPercentage, offerValidUpto } = req.body;
    const foundCategories = await Category.find({}, { name: 1 });
    try {
        ...
        if (!title || !description || !category || !price || !stock || !artist || !color || !images) {
        ...
        } else if (error.message.includes("Product validation failed: title: Path `title`")) {
            hasError = true;
            message = "Product name should be between 3 to 60 characters.";

        } else if (error.message.includes("Product validation failed: title: Product name must not contain special characters")) {
            hasError = true;
        ...
        } else if (error.message.includes("Product validation failed: price: Path `price`")){
            hasError = true;
            message = "Price value should not be negative";

        } else if (error.message.includes("Product validation failed: stock: Path `stock`")) {
            hasError = true;
            message = "Stock value should not be negative";
        
        } else if (error.message.includes("Product validation failed: price: Cast to Number failed for value")) {
            hasError = true;
            message = "PLease enter a valid price";
        } else if (error.message.includes("Product validation failed: stock: Cast to Number failed for value ")) {
            hasError = true;
...
export const editProduct = async (req, res, next) => {
    ...
    const { title, category, description, price, stock, artist, color, offerPercentage, offerValidUpto } = req.body;
    ...
    try {
        const existingProduct = await Category.findOne({ title: { $regex: new RegExp(`^${title}$`, 'i') }})
        .collation({ locale: 'en', strength: 2 });
        ...
        await Product.findByIdAndUpdate(req.params.id, updatedData,{ runValidators: true });
...
export const addSingleImage = async (req, res, next) => { 
    try {
        ...
        if (typeof images !== "undefined") {
            await Product.findByIdAndUpdate(req.params.id, { $push: { images: newProductImage } }, { new: true });
            res.redirect(`/admin/edit-product/${req.params.id}`);
        }
...
export const productOfferAction = async (req, res, next) => {
    try {
        ...
        if (!currentProduct.offerValidUpto || currentDate > currentProduct.offerValidUpto) {
            req.session.message = {
                type: 'danger',
        ...
        await Product.findByIdAndUpdate(req.params.id, { $set: { isOfferActive: state } });
        res.redirect('/admin/products/1');
