...
export const editBanner = async (req, res, next) => {
    ...
    try {
        ...
        if (title && description && type) {
            ...
            await Banner.findByIdAndUpdate(req.params.id, dataToUpdate,{ runValidators: true });
...
export const addBannerImage = async (req, res, next) => { 
    try {
        ...
        if (typeof images !== "undefined") {
            ...
            await Banner.findByIdAndUpdate(req.params.id, { $push: { images: newBannerImage } }, { new: true });
...
export const bannerAction = async (req, res, next) => {
    try {
        ...
        if (state === false) {
            ...
            const activeBanners = await Banner.find({ type: bannerType, isActive: true });
