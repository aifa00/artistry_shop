...
export const getHome = async (req, res, next) => {
    try {
        const allProducts = await Product.find({ softDeleted: false }).populate('category');
        const canvas = allProducts.filter(product => product.category.name === 'Canvas');
        const paintings = allProducts.filter(product => product.category.name === 'Paintings');
        const foundCategories = await Category.find({ disabled: false }).limit(5)
        const heroBanners = await Banner.find({ type: 'hero-banner', isActive: true }).limit(2);
        const featuredBanners = await Banner.find({ type: 'featured-banner', isActive: true }).limit(2);
...
export const getShop = async (req, res, next) => {
        try{            
            ...
            if (categories || colors || price) {
                ...
                //for searching
                const allProducts = await Product.find(queryToFilter).populate('category').sort(queryToSort);

                //resultant product
                foundProducts = await Product.find(queryToFilter).populate('category').sort(queryToSort).skip(skip).limit(pageSize);
                ...
                if (search) {
                    ...
                    const resultantProducts = allProducts.filter(product =>
                        product.title.toLowerCase().includes(searchTerm) ||
                        product.category.name.toLowerCase().includes(searchTerm) 
                    );
...
export const getProduct = async (req, res, next) => {
    try {
        const foundProduct = await Product.findById(req.params.id).populate('category');
...
export const placeOrder = async (req, res, next) => {
    try {
        ...
        if (req.session.couponApplied && req.session.couponApplied.discount !== 0) {
            ...
            const currentCoupon = await Coupon.findById(req.session.couponApplied.couponId);

            const currentDate = Date.now();

            if (currentCoupon.isActive && currentCoupon.expirationDate >= currentDate) {
        ...
        if (req.body.paymentMethod === 'razorpay') {
            ...
            const razorpayOrder = await razorpay.orders.create({
                ...
                amount: walletAmountApplied ? ((grandTotal) - parseInt(walletAmountApplied)) * 100 : (grandTotal) * 100,           
                currency: 'INR',
        ...
        } else if (req.body.paymentMethod === 'cod' || req.body.paymentMethod === 'wallet') {
    ...
    try {
        ...
        if (transactionId && orderId && signature) {
            ...
            if (req.body.referralCode) {
                const foundUser = await User.findOne ({ referralCode: req.body.referralCode });
...
export const returnProduct = async (req, res, next) => {
    try {
        const { orderId, productId, quantity, reason, condition, pickupAddress } = req.body;
        ...
        const foundOrder = await Order.findById(orderId).populate('products.product');
        ...
        //find amt to refund 
        const productReturned = await Product.findById(productId).populate('category');

        let offerPercentage = 0;
        let amountToRefund;

        if (productReturned.isOfferActive && productReturned.offerValidUpto >= new Date ()) {
            offerPercentage += productReturned.offerPercentage;
        }
        if (productReturned.category.isOfferActive && productReturned.category.offerValidUpto >= new Date ()) {
        ...
        if (offerPercentage !==0 && offerPercentage <= 90 ) {
            const discountedPrice = productReturned.price - ((offerPercentage/100) * productReturned.price);
