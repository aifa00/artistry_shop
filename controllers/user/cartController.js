...
export const getCart = async (req, res, next) => {
    try { 
        ...
        for (const cartItem of cartProducts) {
            ...
            if (product.isOfferActive && product.offerPercentage !== null && product.offerValidUpto >= new Date()) {
                
                discountPercentage += product.offerPercentage;
                                    
            }
            if (product.category.isOfferActive && product.category.offerPercentage !== null && product.category.offerValidUpto >= new Date()) {
            ...
            if(discountPercentage !== 0 && discountPercentage <= 90) {
                ...
                totalDiscount += ((discountPercentage/100)*product.price) * cartItem.quantity;
...
export const updateCart = async (req, res, next) => {
    try {
        ...
        if (cartItem) {
            ...
            if(discountPercentage !==0 && discountPercentage <= 90) {
                discountedPrice = product.price - (discountPercentage/100)*product.price;
            ...
            return res.status(200).json({
                ...
                totalPrice: discountedPrice !== undefined ? discountedPrice * cartItem.quantity : product.price * cartItem.quantity,
                subTotal,
                grandTotal: subTotal + 20,
                insufficientStock,
            });
...
export const getCheckout = async (req, res, next) => {
    try {
        ...
        if (currentUser.verified) {
            ...
            const earnedCouponIds = earnedCoupons.map((element) => element.coupon.toString());            
            ...
            if (!insufficientStock) {
                res.render("user/checkout", {
                    ...
                    defaultAddress,
                    totalMrp,
                    couponDiscount: req.session.couponApplied && req.session.couponApplied.discount !== 0 ? req.session.couponApplied.discount : '',
                    offerDiscount: totalDiscount !== 0 ? totalDiscount : '',
                    subTotal,
                    grandTotal,                    
                    error: '',
                ...
                req.session.message = {
                    type :'danger',
                    message: 'Insufficient stock products. PLease check your cart items !',
                }
...
export const applyCoupon = async (req, res, next) => {
    try {
        ...
        if (currentCoupon) {
            const foundCoupon = currentUser.earnedCoupons.find(element => element.coupon.equals(currentCoupon._id) && element.isUsed);
            if (foundCoupon) {
                    ...
                    if (currentCoupon.usedCount < currentCoupon.usageLimit) {
                        if (currentCoupon.minPurchaseAmount <= subTotal) {
                            ...
                            if (currentCoupon.discountType === 'fixedAmount') {
                                ...
                                couponDiscount = (currentCoupon.discountAmount / 100) * subTotal;
                            }                            
...
export const cancelOrder = async (req, res, next) => {
    try {        
        ...
        const foundProduct = foundOrder.products.find((element) => element.product._id.toString() === req.body.productId);
        ...
        if (foundOrder.paymentMethod !== 'cod') {                        
            ...
            if (productCancelled.isOfferActive && productCancelled.offerValidUpto >= new Date ()) {
                offerPercentage += productCancelled.offerPercentage;
            }
            if (productCancelled.category.isOfferActive && productCancelled.category.offerValidUpto >= new Date ()) {
...
export const downloadInvoice = async (req, res, next) => {
    try {
        ...
        const order = await Order.findById(req.query.order).populate('products.product');
