...
router.get("/wishlist", isBlocked, isUser, getWishlist);

router.post("/remove-from-wishlist/:id", isBlocked, isUser, removeFromWishlist);

router.route("/shop").get(isBlocked, getShop).post(isBlocked, getShop);
...
router.get(
  "/single-order/:orderId/:productId",
  isBlocked,
  isUser,
  getSingleOrder
);
...
router.route("/new-password/:id").get(getNewPassword).post(newPassword);
